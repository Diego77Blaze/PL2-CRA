...
nom_p_basic(gnb(Nom,Nom2),N) --> noun(Nom,_,N,_,_), noun(Nom2,_,_,_,_).
nom_p_basic(gnb(Nom),N) --> noun(Nom,_,N,_,_).
nom_p_basic(gnb(D,Nom),N) --> determinant(D,G,N,_,V), noun(Nom,G,N,_,V).
nom_p_basic(gnb(P),N) --> prep_p(P,N).
nom_p_basic(gnb(D,Nom,A),N) --> determinant(D,G,N,_,V), adj_p(A,G,N,V), noun(Nom,G,N,_,_).

prep_p(gp(P,Nom),N) --> preposition(P), noun(Nom,_,N,_,_).
prep_p(gp(P,D,Nom),N) --> preposition(P), determinant(D,G,N,_,_), noun(Nom,G,N,_,_).
...
g_nominal_basico(gnb(Nom,Nom2),N) --> nombre(Nom,_,N,_,_), nombre(Nom2,_,_,_,_).
g_nominal_basico(gnb(Nom),N) --> nombre(Nom,_,N,_,_).
g_nominal_basico(gnb(D,Nom),N) --> determinante(D,G,N,_,_), nombre(Nom,G,N,_,_).
g_nominal_basico(gnb(P),N) --> g_preposicional(P,N).
g_nominal_basico(gnb(D,Nom,A),N) --> determinante(D,G,N,_,_), nombre(Nom,G,N,_,_), g_adjetival(A,G,N).

g_preposicional(gp(P,Nom),N) --> preposicion(P), nombre(Nom,_,N,_,_).
g_preposicional(gp(P,D,Nom),N) --> preposicion(P), determinante(D,G,N,_,_), nombre(Nom,G,N,_,_).
